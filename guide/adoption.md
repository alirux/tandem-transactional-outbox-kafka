...
| A broker Tandem has an adapter for | You publish to Kafka (`tandem-kafka`) or RabbitMQ (`tandem-rabbitmq` 0.1.0, independently versioned, on Tandem 0.10.0 or later) | Another broker needs its own transport adapter: see [Publishing Your Own Message Format](message-format.md) §1 |
