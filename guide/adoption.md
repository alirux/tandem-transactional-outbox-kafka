...
The [README](https://github.com/alirux/tandem/blob/main/README.md#try-it) answers *"does Tandem
...
1. **You already take a pessimistic lock or an optimistic version check on the aggregate root for
   every mutating transaction.** Your existing locking already serializes writers — Tandem's
...
4. **You have no serialization at all today** (no lock, no version, genuinely concurrent blind
   writes). Not a Tandem-specific gap — your domain already tolerates last-write-wins races on
...
| 2 — JPA `@Version` | A five-minute fix once found (flush earlier, or stop supplying a version — `unsequenced()`, §3.2) — but **silent if missed**. |
...
- **On Flyway, or a hand-rolled migration runner, or applying SQL by hand?** Use the generated
  flat file, [`schema/postgres/tandem-baseline.sql`](https://github.com/alirux/tandem/blob/main/schema/postgres/tandem-baseline.sql),
  ...
  ever run multi-instance `LEASE` coordination, but created regardless — there's no smaller
  variant to apply).
...
!!! danger "The hard constraint: the Kafka record key becomes `aggregate_id`, and it is not configurable"
    ...
    affinity beyond simple per-key ordering
    — a Kafka Streams state store keyed by partition, a manually-assigned consumer, a downstream
    system doing its own partition-aware sharding — will see that affinity shift mid-migration.
...
2. Point Tandem at a **new** topic (or a scratch environment) at first, not the production one —
...
3. Once verified, cut Tandem over to the real topic and stop the old producer in the same deploy
   — don't run both against the same topic, or you get true duplicates the CloudEvents `id` (the
...
- **`bucketCount` (default 256) is fine unless you're running a genuinely large fleet of relay
  ...
  ([Known issues](https://github.com/alirux/tandem/blob/main/README.md#known-issues--limitations)),
...
4. Wire the relay (`tandem-spring-relay`, or the manual `WorkerPool` wiring in
   [README — Usage](https://github.com/alirux/tandem/blob/main/README.md#usage)) against staging,
...
adopt without deciding anything (§7), and why the harder cases (§1 shapes 3–4, a hand-rolled
