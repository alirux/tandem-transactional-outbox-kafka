...
| A broker Tandem has an adapter for | You publish to Kafka (`tandem-kafka`) or RabbitMQ (`tandem-rabbitmq`, independently versioned: see [Publishing to RabbitMQ](rabbitmq.md)) | Another broker needs its own transport adapter: see [Publishing Your Own Message Format](message-format.md) §1 |
